# 键盘睡眠
经改装后的 HHKB 键盘，除了功耗远低于原厂外，HHKB QMC 还设计有完善的自动功耗控制程序。用户不再需要为了省电，而进行繁琐的键盘开关机操作。现在已经没有了开关机的概念，只需像鼠标那样，拿起来就用。

下面对键盘的睡眠进行详细介绍，以便用户根据自身需求进行个性化配置。

## 两种睡眠模式
经 HHKB QMC 改装后的 HHKB 键盘支持两种睡眠模式：浅睡眠（Light Sleep Mode）和深睡眠（Deep Sleep Mode）。用户可以结合实际使用需求，通过 [Web App](https://keyboard.xorlink.com) 进行配置。
...
5. 默认出厂设置为浅睡眠，建议保持该设置。当需要将键盘放进背包，可操作快捷键进入深睡眠以防误触。
